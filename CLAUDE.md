...
## Project Overview
Semantic code + git history search CLI. Tree-sitter parsing → pluggable embeddings → LanceDB vector store. Supports local (Ollama) and remote (OpenAI-compatible) embedding providers, local or remote (S3/GCS) storage, and text or JSON output.
...
- State files (`.cortex-recall-state.json`, `.git-search-state.json`) stay local — no cross-machine incremental
...
# JSON output (query, stats, git-search, git-stats, explain)
...
## Data Locations
- `.lance/` — LanceDB storage, `chunks` + `git_history` tables (gitignored); overridable via `storeUri`/`CORTEX_RECALL_STORE_URI`
